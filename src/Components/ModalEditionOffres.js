...
const ModalEditionOffres = ({ isOpen, onClose }) => {
  ...
  const [isDeleteOffreModalOpen, setIsDeleteOffreModalOpen] = useState(false);
  ...
  const confirmDeleteCategory = async () => {
    if (categoryToDelete) {
      ...
      setLocalCategories(localCategories.filter((cat) => cat.id !== categoryToDelete));
      setLocalOffres(localOffres.filter((offre) => offre.categorie_offre_id !== categoryToDelete));
      setSelectedCategory(null);
    }
  ...
  const handleDeleteOffre = (offreId) => {
    ...
    setIsDeleteOffreModalOpen(true);
  };

  const confirmDeleteOffre = async () => {
    if (offreToDelete) {
      await deleteOffre(offreToDelete);
      setLocalOffres(localOffres.filter((offre) => offre.id !== offreToDelete));
    }
    setIsDeleteOffreModalOpen(false);
  ...
  return (
    <>
                ...
                <Dialog.Panel className="w-full max-w-6xl transform overflow-hidden rounded-2xl bg-white p-6 text-left align-middle shadow-xl transition-all">
                  ...
                  <div className="flex flex-col lg:flex-row gap-8">
                    ...
                    <div className="w-full lg:w-1/2">
                      ...
                      <div className="bg-gray-50 p-4 rounded-lg shadow-sm mb-4 max-h-96 md:h-96 overflow-y-auto">
                      ...
                      <form onSubmit={selectedCategory ? handleUpdateCategory : handleAddCategory} className="bg-white p-6 rounded-lg shadow-sm">
                        <h4 className="text-2xl font-semibold mb-4 text-gray-800">{selectedCategory ? "Modifier la catégorie" : "Ajouter une catégorie"}</h4>
                    ...
                    <div className="w-full lg:w-1/2">
                      ...
                      <div className="bg-gray-50 p-4 rounded-lg shadow-sm mb-4 max-h-96 md:h-96 overflow-y-auto">
                        ...
                        {!selectedCategory && (
                          <span className="text-lg font-medium">Choisissez une catégorie pour afficher les offres.</span>
                        )}
                      </div>
                      {selectedCategory && (
                        <form onSubmit={editingOffre ? handleUpdateOffre : handleAddOffre} className="bg-white p-6 rounded-lg shadow-sm h-full">
                          <h4 className="text-2xl font-semibold mb-4 text-gray-800">{editingOffre ? "Modifier l'offre" : "Ajouter une offre"}</h4>
      ...
      <Transition appear show={isDeleteCategoryModalOpen} as={Fragment}>
        <Dialog as="div" className="relative z-50" onClose={() => setIsDeleteCategoryModalOpen(false)}>
          <Transition.Child
            as={Fragment}
            enter="ease-out duration-300"
            enterFrom="opacity-0"
      ...
      {/* Delete Offre Modal */}
      <Transition appear show={isDeleteOffreModalOpen} as={Fragment}>
        <Dialog as="div" className="relative z-50" onClose={() => setIsDeleteOffreModalOpen(false)}>
          <Transition.Child
            as={Fragment}
            enter="ease-out duration-300"
            enterFrom="opacity-0"
                ...
                <Dialog.Title as="h3" className="text-lg font-medium leading-6 text-gray-900">
                  ...
                  <div className="mt-4 flex justify-end">
                    <button
                      type="button"
                      className="inline-flex justify-center rounded-md border border-transparent bg-gray-100 px-4 py-2 text-sm font-medium text-gray-900 hover:bg-gray-200 focus:outline-none focus-visible:ring-2 focus-visible:ring-gray-500 focus-visible:ring-offset-2 mr-2"
                      onClick={() => setIsDeleteOffreModalOpen(false)}
                    >
                      Annuler
                    </button>
                    <button
